...
void ListWidgetWindowItem::WindowState::LayeredWindowAttributes::ApplyAttributes(const HWND& window_handle) const {
    if(IsWindow(window_handle)) {
        SetLayeredWindowAttributes(window_handle, ColorRef, Alpha, Flags);
    }
}

void ListWidgetWindowItem::WindowState::LayeredWindowAttributes::RetrieveAttributes(const HWND& window_handle) {
    if(IsWindow(window_handle) && GetWindowLong(window_handle, GWL_EXSTYLE) & WS_EX_LAYERED) {
        GetLayeredWindowAttributes(window_handle, &ColorRef, &Alpha, &Flags);

        // If the alpha mode isn't turned on, an alpha mode probably wasn't set, so manually ensure
        // that the alpha is 255 so that the window doesn't start transparent if alpha is turned on.
        if(Flags != LWA_ALPHA) {
            Alpha = 0xFF;
        }
    } else {
...
bool ListWidgetWindowItem::WindowState::LayeredWindowAttributes::operator==(const ListWidgetWindowItem::WindowState::LayeredWindowAttributes& other) const {
...
ListWidgetWindowItem::WindowState::LayeredWindowAttributes::LayeredWindowAttributes(const HWND& window_handle) {
    RetrieveAttributes(window_handle);
}

ListWidgetWindowItem::WindowState::LayeredWindowAttributes::LayeredWindowAttributes() {
    Clear();
}
...
bool ListWidgetWindowItem::WindowState::ApplyState(const HWND& window_handle) const {
    if(IsWindow(window_handle)) {
...
bool ListWidgetWindowItem::WindowState::operator==(const ListWidgetWindowItem::WindowState& other) const {
